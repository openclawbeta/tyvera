...
function shortWallet(rank) {
  const base = `5F${(100000 + rank * 7919).toString(16).padStart(10, "0")}`;
  return `${base}...${(9999 - rank).toString().padStart(4, "0")}`;
}

export function buildHolderIntelSnapshot(subnets) {
  const topHolders = Array.from({ length: 100 }).map((_, i) => {
    const rank = i + 1;
  ...
  const subnetFlows = subnets.slice(0, 8).map((subnet, idx) => {
