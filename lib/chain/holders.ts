...
const MAX_HOTKEYS_PER_COLDKEY = 4;
...
async function getColdkeysByIndex(api: ApiPromise): Promise<string[]> {
  ...
  for (let i = 0; i < Math.ceil(MAX_COLDKEYS_TO_SCAN / 2); i++) {
    try {
...
async function getColdkeysFromMetagraph(api: ApiPromise, netuids: number[]): Promise<string[]> {
  ...
  for (const netuid of sampleNetuids) {
    try {
      const nRaw = await withTimeout(api.query.subtensorModule.subnetworkN(netuid), 3_000, `subnetworkN(${netuid})`);
      const n = toNumber(nRaw);
      const uids = Array.from({ length: Math.min(n, 64) }, (_, i) => i);

      const hotkeys = await Promise.all(
        uids.map((uid) =>
          withTimeout(api.query.subtensorModule.keys(netuid, uid), 1_500, `keys(${netuid},${uid})`).then(String).catch(() => ""),
        ),
      );
  ...
  for (const hotkey of topHotkeys) {
    try {
      const owner = await withTimeout((api.query.subtensorModule as any).owner(hotkey), 1_500, `owner(${hotkey.slice(0, 10)})`);
...
export async function fetchHolderAttributionFromChain(limit = 250): Promise<HolderAttributionSnapshot> {
  ...
  try {
    ...
    const sorted = positions.sort((a, b) => b.stakeTao - a.stakeTao).slice(0, limit);
