...
export interface LandingInitialData {
  ...
  rootAnnual: number | null;
  topDailyPct: number | null;
  ticker: { name: string; yield: string; up: boolean }[];
}
...
export default function HomeClient({ initial }: HomeClientProps) {
  const [ticker, setTicker] = useState(initial?.ticker?.length ? initial.ticker : TICKER_FALLBACK);
  const [featured, setFeatured] = useState<FeaturedPick[]>(initial?.featured ?? []);
  const [rootAnnual, setRootAnnual] = useState<number | null>(initial?.rootAnnual ?? null);
  const [topDailyPct, setTopDailyPct] = useState<number | null>(initial?.topDailyPct ?? null);
            ...
            <FadeIn delay={0.1}>
              <div className="glass p-7">
                ...
                <p
                  ...
                  style={{ color: "var(--aurora-sub)" }}
                >
                  <strong style={{ color: "var(--aurora-ink)" }}>τ (TAO)</strong>{" "}
                  is Bittensor&rsquo;s native token. More τ staked into a subnet
                  means deeper liquidity and a lower-risk exit — the key signal
                  behind these bands.
                </p>
